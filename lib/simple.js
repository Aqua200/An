  ...
  const sock = Object.defineProperties(conn, {
    ...
    sendNyanCat: {
      async value(jid, text = '', buffer, title, body, url, quoted, options) {
        ...
        const prep = generateWAMessageFromContent(jid, { extendedTextMessage: { text: text, contextInfo: { externalAdReply: { title: title, body: body, thumbnail: buffer, sourceUrl: url }, mentionedJid: [] } } });
    ...
    getFile: {
      ...
      async value(PATH, saveToFile = false) {
        let res;
        let filename;
        const data = Buffer.isBuffer(PATH) ? PATH : PATH instanceof ArrayBuffer ? PATH.toBuffer() : /^data:.*?\/.*?;base64,/i.test(PATH) ? Buffer.from(PATH.split`,`, [1], 'base64') : /^https?:\/\//.test(PATH) ? await (res = await fetch(PATH)).buffer() : fs.existsSync(PATH) ? await fs.promises.readFile(PATH) : typeof PATH === 'string' ? PATH : Buffer.alloc(0);
